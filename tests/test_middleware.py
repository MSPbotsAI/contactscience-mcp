...
from contactscience_mcp.__main__ import _build_http_app
from contactscience_mcp.config import Settings
from contactscience_mcp.server import create_mcp_server, get_client_from_context
...
def test_missing_header_returns_401_with_required_headers_listed():
    ...
    with TestClient(app) as client:
        ...
        body = resp.json()
        assert "X-ContactScience-Authorization" in body["required_headers"]
...
def test_header_present_reaches_request_context(monkeypatch):
    ...
    from contactscience_mcp.server import GatewayTokenMiddleware, _gateway_creds_var

    settings = Settings()
    seen = {}

    async def fake_app(scope, receive, send):
        seen["authorization"] = _gateway_creds_var.get()
    ...
    async def run():
        scope = {
            "type": "http",
            "path": "/mcp",
            "headers": [(b"x-contactscience-authorization", b"test-token-123")],
        }
    ...
    asyncio.run(run())
    assert seen["authorization"] == "test-token-123"
